...
def activity(bot: Bot):
    ...
    try:
        onchain_instance = Onchain(bot.account, Chains.CHAINBASE_TESTNET)
        nonce = onchain_instance.get_tx_count(address=bot.account.address)
        excel_report.set_cell(f'Txs Count', f'{nonce}')
        logger.info(f'Данные занесены в таблицу ChainbaseActivity.xls!')
    except Exception as e:
        logger.error(f'Ошибка в сети {Chains.CHAINBASE_TESTNET.name.upper()}: {e}')
